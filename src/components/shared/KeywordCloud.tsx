...
const KeywordCloud = () => {
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2/3 h-2/3 bg-[#0055A4]/8 blur-[120px] -z-10" />

      <div className="text-center mb-12">
        <span className="inline-block px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-[0.2em] text-[#3B82F6] mb-4">
      ...
      <div className="flex gap-3 flex-wrap justify-center mb-8 max-w-6xl mx-auto px-4">
        {FOOTER_PAGES.slice(0, 16).map((link, idx) => (
          <Link
            key={idx}
            href={link.href}
            className="px-4 py-2 rounded-xl bg-white/3 border border-white/10 text-xs font-bold text-slate-300 hover:text-white hover:bg-white/[0.08] hover:border-[#0055A4]/40 hover:shadow-[0_0_20px_rgba(59,130,246,0.2)] transition-[color,background-color,border-color,box-shadow,transform] duration-300 cursor-pointer active:scale-95"
          >
            {link.label}
          </Link>
        ))}
      ...
      {FOOTER_PAGES.length > 16 && (
        <details className="group max-w-6xl mx-auto px-4 mb-16">
          <summary className="list-none text-center cursor-pointer mb-8">
            <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/3 border border-white/10 text-xs font-bold text-slate-400 hover:text-white hover:border-[#0055A4]/40 transition-all duration-300 group-open:border-[#0055A4]/40 group-open:text-[#60A5FA]">
          ...
          <div className="flex gap-x-5 gap-y-3 flex-wrap justify-center opacity-60 group-open:opacity-100 transition-opacity duration-500">
            {FOOTER_PAGES.slice(16).map((link, idx) => (
              <Link
                key={idx}
                href={link.href}
                className="text-[11px] font-medium text-slate-500 hover:text-[#3B82F6] transition-colors whitespace-nowrap cursor-pointer hover:underline decoration-[#0055A4]/40 underline-offset-4"
              >
                {link.label}
              </Link>
            ))}
